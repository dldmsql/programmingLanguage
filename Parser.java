...
public class Parser { // input string > parsing > return AST
    ...
    // <decl>  -> <type> id [=<expr>]; 
	    ...
	    String id = match(Token.ID); // 변수 이름 매치
	    Decl d = null;
    ...
    // <stmt> -> <block> | <assignment> | <ifStmt> | <whileStmt> | ...
        ...
        switch (token) {
	    case SEMICOLON:
            match(token.SEMICOLON); return s; // Assign 파싱
        case LBRACE:			
	        match(Token.LBRACE);		
            s = stmts();
            match(Token.RBRACE);	
        ...
        case IF: 	// if statement 
            s = ifStmt(); return s; // if 파싱
        case WHILE:      // while statement  
            s = whileStmt(); return s; // while 파싱
    ...
    private Let letStmt () {
        ...
        match(Token.IN);
        Stmts ss = stmts(); // let문 block 문장 파싱
        match(Token.END);
        match(Token.SEMICOLON);
        return new Let(ds, null, ss); // 가운데 null은 function
    }
    ...
    private Stmt assignment() {
        ...
        Expr e = expr(); // 수식 파싱
        match(Token.SEMICOLON); // 세미콜론 매치
        return new Assignment(id, e); // 리턴 AST
    }
    ...
    private If ifStmt () {
        ...
        match(Token.THEN);
        Stmt s1 = stmt();
        Stmt s2 = new Empty(); // 빈 복합문 AST 생성
        if (token == Token.ELSE){
            match(Token.ELSE);
            s2 = stmt(); // else가 있으면 복합문 AST 생성
        }
    ...
    private Expr aexp () { // 산술연산
        // <aexp> -> <term> { + <term> | - <term> }
        // aexp implementation
        Expr e = term(); // 첫번째 항 파싱
    ...
    private Expr factor() {
        ...
        if (token == Token.MINUS) 
            op = new Operator(match(Token.MINUS)); // 음수는 Unary만
