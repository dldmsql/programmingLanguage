...
public class Sint {
    ...
    State Eval(Assignment a, State state) {
        ...
        Value v = V(a.expr, state); // a는 Expr를 갖기 때문에, V()로 value 계산이 가능
        return state.set(a.id, v); // id는 이미 스택에 저장되어 있기 때문에 id를 찾아서 v만 저장하고 변한 상태 리턴
    }

    // (2) If Eval Implementation
    State Eval(If c, State state) {
        // If Implementation
        if(V(c.expr,state).boolValue())  // boolValue()는 expr의 계산 결과가 boolean인지 판별용
    ...
    State Eval(While l, State state) {
        ...
        if(V(l.expr, state).boolValue()) 
            return Eval(l,Eval(l.stmt,state)); // 재귀적으로 구현
        else 
            return state;
    }
    ...
    State Eval(Let l, State state) {
        // Let Implementation
        State s = allocate(l.decls, state); // allocate()는 push로 구현. 현재 상태에 decls 추가
        s = Eval(l.stmts, s); // let문의 복합문에 대한 해석을 수행
        return free(l.decls,s); // END를 만나면 decls에서 선언한 변수를 제거. pop으로 구현
    }
    
    // (5) Read Eval Implementation
    State Eval(Read r, State state) {
        // Read Implementation
        if(r.id.type == Type.INT){ // id의 타입을 확인해야 scanner에서 그에 맞는 함수 호출이 가능해진다.
            int i = sc.nextInt();
    ...
    State allocate (Decls ds, State state) { // 선언된 변수들을 위한 엔트리들을 상태에 추가
        // Allocate
        for(Decl d : ds){
    ...
    State free (Decls ds, State state) { // 선언된 변수들의 엔트리를 상태에서 제거
	    // Free Implementation
    ...
    State allocate (Decl ds, State state) { // 선언된 변수들을 위한 엔트리들을 상태에 추가
        // Allocate Implementation
        state.push(ds.id, new Value(ds.expr));
        return state;
    }

    //  Free Function Implementation
    State free (Decl ds, State state) { // 선언된 변수들의 엔트리를 상태에서 제거
        // Free Implementation
        state.pop();
        return state;
    }
